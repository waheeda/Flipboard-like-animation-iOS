...
-(MPFlipViewControllerDirection) getDirectionFromVelocity:(CGPoint) velocity
{
  // implementation for vertical animation only

    if(velocity.y > 0)
        return MPFlipViewControllerDirectionReverse;
    else
        return MPFlipViewControllerDirectionForward;
}
...
//		if(vel.y > 0)  //reverse direction //(value <= MARGIN)
//		{
//			if (![self startFlipWithDirection:MPFlipViewControllerDirectionReverse])
//				return;
//		}
//		else if  (vel.y < 0) // forward direction   //(value >= dimension - MARGIN)
//		{
...
//		}
        ...
        if (![self startFlipWithDirection:[self getDirectionFromVelocity:vel]])
            				return;
...
//		CGFloat value = isHorizontal? tapPoint.x : tapPoint.y;
//		CGFloat dimension = isHorizontal? self.view.bounds.size.width : self.view.bounds.size.height;
        return   YES;  //(value <= MARGIN || value >= dimension - MARGIN);
	}
